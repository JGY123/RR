...
**2026-05-04 verification on freshly-parsed FORMAT 4.3 data:** F19 fix shipped correctly — `pct_specific` / `pct_factor` keys are now present on every `hist.summary[]` entry (verified via `/tmp/f19_test_output.json` re-parse of `~/Downloads/risk_reports_sample.csv`). However, **all per-period values are `null` upstream** — FactSet's RiskM section does not ship per-historical-period `% Stock Specific` / `% Factor Risk` in the current CSV format (documented in parser at L1462-1467: "FactSet folded RiskM → 18 Style Snapshot, but did NOT carry % Stock Specific / % Factor Risk forward at portfolio level"). Tier 1 therefore fires zero times in current data; **Tier 2 (MCR-derived) is the canonical source of truth today**, with `Σ|sector mcr| = sum.pct_specific` matching exactly (Δ = -0.00 on GSC, IDM, ISC at latest week 2026-04-30). F19 remains correct discipline and will activate Tier 1 automatically if FactSet resumes shipping per-period pct_specific. F18 letter mentions this state.
