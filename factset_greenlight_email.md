...
First — thank you. The work to get sample-3 to where it is has been substantial, and we really appreciate it. The output is clean, the parser sings, and we're ready to go.
...
- We expect a clean pass — sample-3 already confirmed the per-period column structure works — but inception-depth is the first time we'd see the wide format at full scale.
...
These are columns that are constant or empty across every period and add noise without value. All easy to remove:

**1. `%T_Check` on the group tables** — Sector Weights, Industry, Country, Region, Overall, REV. The column is constant 1.00 (or empty) for every row across all 6 periods. Country shows it as something like `%T filter` — same idea, please remove. Save once, apply to every group table.

**2. `% Factor Contr. to Tot. Risk: Local`** on the Security section — this column is empty for every holding across every period. The "Local" macro factor isn't computed in the worldwide model, so the column should be dropped.

**3. Hidden Industry rows.** Industry table has incomplete benchmark coverage — Σ bench weight only sums to 68–79% across strategies (vs 100% on Sector / Country / Region). About a quarter of the bench is hidden. Please unhide and save before the GSC run. I tried it this morning without saving so I know it's a quick fix — just unhide, save, and run.

These three should each be a few minutes — please do them ahead of GSC.
...
**2. Five static per-holding fields are repeated 6× per row in the Security section.** They live inside the weekly column block but never change week-to-week:

- Redwood GICS Sector / Redwood Region1 / Redwood Country / Industry Rollup / RWOOD_SUBGROUP
- *(Also Price Volatility appears static per holding in our sample — please confirm whether it actually changes weekly or not. If static, include it in the dedup.)*

Three options, ordered by preference:

- **(a) Move them to a fixed-prefix block** (single occurrence per row). ~10% file-size reduction.
- **(b) Ship them as a separate one-time / annual file** keyed by SEDOL. Weekly massive file omits them; I join client-side. Same pattern as our existing reference-mapping setup — happy to take this approach if (a) isn't easy.
