...
## Normalize() remap skips non-holdings collections (NEW — surfaced 2026-04-23, cardUnowned)
Pattern B variant. `normalize()` L562–L573 remaps parser field names on `st.hold` only: `hn.b ?? hn.bw ?? 0`, `hn.tr ?? hn.pct_t ?? 0`, etc. **`st.unowned` rows retain parser's original key names** (`bw`, `pct_t`, `aw`, `pct_s`) — so any render reading the normalized names (`u.b`, `u.tr`) silently gets `undefined`. Detection heuristic: when a tile ingests a parser collection other than `st.hold[]` (e.g. `st.unowned`, `st.watchlist`, `st.benchOnly`), check whether normalize() applies the same field-name remap to that collection, or the render will silently read undefined. Defensive fix in render is `u.b ?? u.bw` fallback chain; permanent fix is to mirror the remap inside normalize.

## Parser bucket semantics ≠ tile semantics (NEW — surfaced 2026-04-23, cardUnowned)
Aspirationally-named parser buckets (`unowned`, `orphan`, `benchmark-only`) are partitioned by a *technical condition*, not the narrative meaning. `_extract_security` L639 puts a row into `st.unowned` iff **`w`/`bw`/`aw` are ALL None** — i.e. the bucket is "securities with no weight data", not "benchmark-unowned constituents". Data probe confirmed: `bw=None, pct_t=None` on 100% of rows across all 7 strategies (22 rows total = rights, cash detached, orphan FactSet entries). The tile's narrative ("stocks contributing most to TE") is structurally impossible to satisfy from this bucket. Audit heuristic: **for every parser collection consumed by a tile, read the partition CONDITION from the parser source, not the bucket name**. Cross-check narrative against partition.

## Sign-collapse — 4th site confirmed (ESCALATED — surfaced 2026-04-23, cardUnowned)
Not a `Math.abs` instance this time but the same pattern class: cardUnowned L1994 `color:var(--neg)` **unconditionally** on the TE Contrib cell, regardless of sign. Adds a 5th site to the B74 shared-helper scope (B74 already covers: cardFRB L2696, oDrRiskBudget L5389, Risk Decomp Tree L2770, cardRiskFacTbl MCR-to-TE L2996+drill KPI L3526). Trivial softening applied (`var(--neg)` → `var(--txth)` neutral pending PM policy); permanent decision rolls into B74. **Detection grep:** `color:var(--neg)` without surrounding `?:` ternary deserves a sign-correctness check.

## Synthesis tiles invisible-until-populated (NEW — surfaced 2026-04-23, cardWatchlist)
Synthesis / curated-content tiles that early-return `''` when their input is empty silently hide the feature from new users. Seen: cardWatchlist (flag-based curation, fixed in Batch 6 via empty-state shell with onboarding copy), cardThisWeek (what-changed diff, empty on first load), riskAlerts (threshold-triggered). Audit heuristic: **if a tile's render function returns `''` based on empty user-generated state, replace the early return with an empty-state card showing the feature's entry-point copy**. Affects ≥4 tiles.

## Mutation handlers that don't re-render dependent tiles (NEW — surfaced 2026-04-23, cardWatchlist)
Out-of-tile buttons that mutate a shared state container (e.g. `_holdFlags`, `_treeDrill`, `_facView`, `_selectedWeek`) must trigger a re-render of every tile whose view depends on that state. Site: `cycleFlag` L3667 writes to `_holdFlags` + localStorage but only patched the clicked `.flag-btn` — `cardWatchlist` on Overview tab stayed stale until next `upd()`. Fixed in Batch 6 by appending an in-place `wlCard.replaceWith(watchlistCardHtml(cs))` to `cycleFlag`. **Audit heuristic:** grep every `onclick` that writes to `_X`, then grep every `getElementById` / render-fn reading `_X` — for each pair, confirm the mutation triggers re-render of the dependent tile.

## Ghost-data anti-pattern: localStorage refs to removed entities (NEW — surfaced 2026-04-23, cardWatchlist)
User-generated state (flags, notes, alerts) keyed by an entity ID (ticker, factor name) that may have left the underlying dataset. cardWatchlist flagged tickers that exit the portfolio rendered `0.00/0.00` indistinguishable from a real zero-weight position. Fix: presence-check at render time and render `—` placeholders + muted opacity + `EXITED` chip for absent entities. Likely also affects: saved note keys referencing removed tickers, saved custom-alerts tied to deprecated factor names. **Heuristic:** any localStorage key referencing an entity from `cs.hold[]` / `cs.factors[]` / `cs.sectors[]` needs a presence-check at render.

## Completed audits (append-only — 2026-04-23 batch 6)
- ✅ cardBenchOnlySec (2026-04-23, audit only — GREEN/YELLOW/GREEN; top finding = PNG button violates user policy (removed), missing card-title note-hook (added), `data-col` + sort on Biggest Missed col 3 added; "% of benchmark missed" denominator ambiguity → B79 PM gate; 6 trivial applied, 1 non-trivial → B79)
- ✅ cardUnowned (2026-04-23, audit only — **RED/YELLOW/YELLOW**; top finding = tile structurally unable to show what title promises — parser `st.unowned` bucket is "rows with no weight data", not "benchmark-unowned TE contributors"; data probe confirmed `bw=pct_t=None` on 100% of 22 rows across 7 strategies; normalize() skips `st.unowned` remap (Pattern B variant); 4th sign-collapse site escalated to B74; 7 trivial applied, 4 deferred pending B80 data-source decision, 4 non-trivial → B80–B83)
- ✅ cardWatchlist (2026-04-23, audit only — YELLOW/**RED**/YELLOW; top finding = RED T2 = no `<thead>`, no sort, no CSV, no empty-state, no remove affordance, `cycleFlag` didn't re-render dependent tile — all fixed except remove-×; ghost holdings (flagged ticker exited) now render with `EXITED` chip + muted opacity; `FLAG_COLORS` tokenized; 12 trivial applied, 4 non-trivial → B84–B87)

## Sign-collapse — 5th & 6th sites confirmed (ESCALATED — 2026-04-24, cardFacContribBars)
cardFacContribBars adds a 5th render site: `Math.abs(f.c)` across 4 inline call sites (L3153 checkbox data-c, L3347 shadow-max, L3353 TE bar x, L3378 Both-mode TE bar) plus the chart's Both-mode overlay of signed Exposure against magnitude-only `|TE Contrib|` — narratively inconsistent whenever `f.c<0`. AUDIT_LEARNINGS sign-collapse ledger now reads: cardFRB L2696, oDrRiskBudget L5389, Risk Decomp Tree L2770, cardRiskFacTbl MCR-to-TE L2996 + drill KPI L3526, cardUnowned L1994 (static-red), cardFacContribBars (5 inline `Math.abs` + dual-axis overlay). **6 tiles.** Shared-helper recommendation (B74) graduates from "strongly recommended" to "required before next Risk-tab UX pass". Trivial softening (relabel "TE Contrib %" → "|TE Contrib %|" in Both mode) applied in cardFacContribBars as a narrative disclaimer until B74 lands.

## Anonymous Risk-tab cards sweep — 2-of-4 converted (2026-04-24, Batch 7)
B78 scope progress: `cardFacContribBars` (L3114 wrapper) and `cardTEStacked` (L3096 wrapper) now have stable ids + note-hooks. Remaining anonymous Risk-tab wrappers to address in a post-closeout sweep: Beta History at L3105 (needs `id="cardBetaHist"` — already has onclick drill, just missing id+note-hook), Factor Exposure History at L3194 (needs `id="cardFacHist"` — has complex toggle state that deserves its own audit), Factor Correlation Matrix at L3207 (IS the live body of `cardCorr`, which placeholder sits at L1299 — ghost-tile retire decision is B59). Strategy: remaining 2 tiles (cardBetaHist + cardFacHist) close B78 in one post-closeout structural commit, not as full tile audits, since they're mostly id-assignment + note-hook + CSS-addressability.

## Non-finding discipline (NEW — 2026-04-24, cardAttribWaterfall)
Confirmed **not** a B73 or B74 site. Tile consumes only `f.imp` (signed return attribution, unambiguous by construction — factor return × exposure). `Math.abs` appears at L1355 for axis-range setup only, not for display. Explicit "not a Nth site" findings are worth recording — it bounds the scope of open refactors (B73 stays at 3 sites, not 4; B74 stays at 5 sites before this audit, now 6 via cardFacContribBars). Audit heuristic: at every factor-family tile, explicitly assert whether it's a B73/B74 candidate with 1-sentence justification — prevents silent scope creep on shared refactor items.

## Parser-synthesized heuristics pollute downstream tiles (NEW — 2026-04-24, cardTEStacked)
`rRisk()` L3014 computes `factorRisk = Σ|f.c|/totalTE * 1.2` (capped 0.85) as a fallback when per-week `pct_specific`/`pct_factor` missing. This heuristic carries a fudge coefficient (`* 1.2`) with no documentation or UI disclaimer. Every consumer of `factorRisk`/`idioRisk` (cardTEStacked stacked-area normalization, Risk Decomp Tree, Risk KPI row) inherits it silently. Three separate consumers; one synthesized source. Audit heuristic: for any variable computed via `.filter` / `.reduce` / `Math.min/max` at the top of a render fn, check whether it's PARSER-derived (authoritative) or RENDER-synthesized (heuristic) before reading into downstream charts. Synthesized vars deserve `/* synthesized — not authoritative */` comments and UI disclaimer. Pattern-named **"Synthesized fallback leakage"** — distinct from Pattern C (render-side re-derivation from wrong config) because here there is no config to derive from; the value is made up.

## Factor-tile semantic ambiguity: `pct_specific`/`pct_factor` is "% of Total Risk" not "% of TE" (NEW — 2026-04-24, cardTEStacked)
Per `CSV_STRUCTURE_SPEC.md` L184, FactSet's `pct_specific`/`pct_factor` columns decompose **Total Risk** (variance-family) not **TE** (linear). cardTEStacked at L3253 multiplies `h.te * (raw_s/tot)` as if these were TE-shares, producing a visually plausible but mathematically dubious stacked area (variance budget ≠ TE budget under sqrt transform). Confirmed RED finding with PM gate (B96). This semantic class joins B74's factor-tile-semantics cleanup family — expect similar issues in Risk Decomp Tree (L2770) and anywhere else consuming `pct_*` columns. Audit heuristic: **any FactSet-sourced percentage column must have its denominator identified before rendering as a share of something on screen**. Never assume `pct_X` → `X%`.

## Completed audits (append-only — 2026-04-24 batch 7)
- ✅ cardAttribWaterfall (2026-04-24, audit only — GREEN/YELLOW/YELLOW; top finding = clean tile, explicitly NOT a 6th B73/B74 site; 3 trivial applied (CSV button + helper + note-hook tooltip addendum), 1 deferred FS variant, 2 non-trivial → B88–B89)
- ✅ cardFacContribBars (2026-04-24, audit only — **RED/RED/YELLOW**; top findings = anonymous card (id assigned per B78), 6th B74 sign-collapse site, 4th B73 active-vs-raw in-Both-mode overlay, threshold slider silently overridden by group-pill; 10 trivial applied (id, note-hook, plotly_click, 4× #fb923c tokenized, hex2rgba for 3 rgba sites, threshold intersect in setFacGroup+setFacBarAll, FAC_PRIMARY filter dropped from Both mode, dead --riskFacMode custom-prop removed, radio-group a11y, Both-mode label relabeled "|TE Contrib %|"), 6 deferred (CSV, persistence, data probe, FS, --pri-alt token, full B73/B74 resolution), 6 non-trivial → B90–B95)
- ✅ cardTEStacked (2026-04-24, audit only — **RED/RED/YELLOW**; top finding = pct_specific/pct_factor interpreted as TE-shares but CSV_STRUCTURE_SPEC says "% of Total Risk not TE" — mathematically dubious stacked decomposition (B96); synthesized `factorRisk`/`idioRisk` heuristic pollutes 3 consumer tiles (B99); 7 trivial applied (id, note-hook, plotly_click → oDrMetric('te'), empty-state write-through, rangeslider+fill colors tokenized via _teStackColors helper, CSV export), 2 deferred (_selectedWeek marker, rangeslider persistence), 5 non-trivial → B96–B100)

## 🎯 ALL 21 TIER-1 TILES AUDITED + TRIVIAL FIXES APPLIED (2026-04-24)
Completing Batch 7 closes the initial tile-audit phase. **21 tiles at `.v1.fixes`, 0 signed off.** Next phase = review marathon (per user's Option 3 cadence): each tile reviewed in-browser with screenshot + changes-made + outstanding PM gates for explicit OK. Open PM-gate queue includes (priority-ranked by impact on user-facing accuracy):
1. **B80** cardUnowned data-source decision — whole tile dead without it
2. **B96** cardTEStacked pct_* denominator semantics — mathematically dubious stack
3. **B73** active-vs-raw unification (4 sites)
4. **B74** sign-collapse shared-helper (6 sites)
5. **B61** cardGroups ORVQ rank taxonomy
6. **B20/B39** MCR+Scatter rename pair
7. Remaining: B59 ghost-tile, B79 label, B86/B87 watchlist UX, B89 waterfall naming, B88/B97/B70 week-selector sweep, B99 heuristic audit, B100 palette tokens, B90–B95 cardFacContribBars follow-ups
